...
CMDs[#CMDs + 1] = {NAME = '', DESC = 'NOW THIS IS WHERE THE MODDING BEGINS'}
CMDs[#CMDs + 1] = {NAME = 'unc', DESC = 'Runs an UNC test'}
CMDs[#CMDs + 1] = {NAME = 'soluna', DESC = 'Runs Soluna script'}
CMDs[#CMDs + 1] = {NAME = 'flicker (CLIENT)', DESC = 'Makes your avatar flickers'}
CMDs[#CMDs + 1] = {NAME = 'overdrive', DESC = 'Changes your speed mid-walk'}
CMDs[#CMDs + 1] = {NAME = 'reversemovement / rmove', DESC = 'Invert all your movements'}
...
addcmd('soluna',{},function(args, speaker)
notify("Loading", "Hold on a sec")
loadstring(game:HttpGet("https://raw.githubusercontent.com/EndOverdosing/Soluna-API/refs/heads/main/rivals-rewrite.lua",true))()
end)

addcmd('flicker',{},function(args, speaker)
notify('NOTICE', 'This will only lasts 1 second.')
wait(1)
...
addcmd('overdrive', {}, function(args, speaker)
notify('NOTICE', 'This will only lasts 1 and a half seconds')
    local hum = speaker.Character and speaker.Character:FindFirstChildOfClass("Humanoid")
    if hum then
        for i = 1, 5 do
            hum.WalkSpeed = math.random(50,150)
            task.wait(0.3)
        end
    end
end)
...
addcmd('reversemovement', {'rmove'}, function(args, speaker)
    local char = speaker.Character
    if not char then return end
    ...
    if reverseConnection then reverseConnection:Disconnect() end

    reverseConnection = RunService.RenderStepped:Connect(function()
        hum:Move(-hum.MoveDirection, true)
    end)

    notify("rmove", "Reverse ON 😈")
end)

addcmd('normalmovement', {'nmove'}, function()
    if reverseConnection then
        reverseConnection:Disconnect()
        reverseConnection = nil
    end
    notify("nmove", "Back to normal 🙂")
end)

local slipConn

addcmd('slipperyice', {'sice'}, function(args, speaker)
    local char = speaker.Character
    if not char then return end
    ...
    if slipConn then slipConn:Disconnect() end

    slipConn = RunService.RenderStepped:Connect(function()
        ...
        if move.Magnitude > 0 then
            ...
            root.Velocity = root.Velocity + (move * 20) + randomDir
        end
    end)

    if notify then
        notify("sice", "Iceskate removed, slippery level: max 😈")
    end
end)

addcmd('unslipperyice', {'unsice'}, function()
    if slipConn then
        slipConn:Disconnect()
        slipConn = nil
    end
    ...
    if notify then
        notify("unsice", "You remembered how to iceskate 🙂")
    end
end)
...
addcmd('sink', {}, function(args, speaker)
    local char = speaker.Character
    if not char then return end
    ...
    if notify then
        notify("sink", "You are sinking… 🕳️")
    end
end)

addcmd('keysystem',{'key'},function()
local correctKey = "iymodded" -- same key as your website
...
submit.Position = UDim2.new(0.5, -100, 0.45, 0)
submit.Text = "Submit"
submit.Parent = frame

-- Website Button (visible from start)
local websiteBtn = Instance.new("TextButton")
websiteBtn.Size = UDim2.new(0, 200, 0, 40)
websiteBtn.Position = UDim2.new(0.5, -100, 0.7, 0)
websiteBtn.Text = "Get Key Website"
...
secretFrame.Size = UDim2.new(0, 300, 0, 150)
secretFrame.Position = UDim2.new(0.5, -150, 0.75, 0)
secretFrame.BackgroundColor3 = Color3.fromRGB(50, 50, 50)
secretFrame.Visible = false
secretFrame.Parent = gui

-- Secret Label
local secretLabel = Instance.new("TextLabel")
secretLabel.Size = UDim2.new(1, 0, 1, 0)
secretLabel.Text = "Welcome 👀"
...
websiteBtn.MouseButton1Click:Connect(function()
	toClipboard("iyfaithkey.netlify.app")
end)
...
addcmd("unsink",{}, function()
    ...
    if notify then
        notify("unsink", "Back on solid ground 🙂")
    end
end)

addcmd('kick',{},function()
Players.LocalPlayer:Kick("You kicked yourself.")
end)
...
notify("UNC test", "Done! Please check the developer console")
end)
